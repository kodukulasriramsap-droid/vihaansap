...
export default function TodaysSession() {
  ...
  const now = Date.now(); 
  const sessions = enrolledBatches.map(batch => {
    const batchSessions = (db.batchSessions || []).filter(session => 
      session.batchId === batch.id && isTargetedToStudent(session, studentProfile)
  ...
  }).filter(Boolean).sort((a, b) => new Date(b.sessionDateTime || b.createdAt || 0).getTime() - new Date(a.sessionDateTime || a.createdAt || 0).getTime());
